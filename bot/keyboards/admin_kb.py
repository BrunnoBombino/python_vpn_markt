...
def get_user_manage_keyboard(username: str, is_enabled: bool) -> InlineKeyboardMarkup:
    ...
    buttons = [
        [InlineKeyboardButton(text="➕ Выдать 5 дней подписки", callback_data=f"adm_add_5:{username}")],
        ...
        [InlineKeyboardButton(text="❌ Удалить аккаунт навсегда", callback_data=f"adm_delete_user:{username}")], # <-- ДОБАВЛЕНО
